...
Matrix initMatrix(int row, int col)
{
    ...
    mat.row = row;
    mat.col = col;
    mat.values = (float*)malloc(row*col*sizeof(float));
    int i = 0;
            ...
            for (int filterRow = 0; filterRow < filter.row; filterRow++)
            {
                for (int filterCol = 0; filterCol < filter.col; filterCol++)
                {
                    ...
                    else
                    {
                        ...
                        MatValue(out, row, col) += MatValue(matrix, row + filterRow - offset, (col + filterCol - offset)) * MatValue(filter, filterRow, filterCol);
                    }
                } 
            }
        }  
