...
numImage = int(input("#image : "))
nbImg = int(input("entrez le nombre d'images de test : "))

lib.write('#define NB_IMG   {}\n'.format(nbImg))

lib.write("float weight1[{}][{}]={{".format(len(layer1Weights), 25))
...
lib.write("float weight2[{}][{}][{}]={{".format(len(layer2Weights),16, 25))
for k in range(len(layer1Weights)):
    lib.write("\n\t\t\t\t\t\t ")
...
lib.write("float weight3[{}][{}]={{".format(len(layer3Weights), len(layer3Weights[0])))
...
lib.write("float bias1[{}]={{".format(len(layer1bias)))
...
lib.write("float bias2[{}]={{".format(len(layer2bias)))
...
lib.write("float bias3[{}]={{".format(len(layer3bias)))
...
lib.write("float testValues[{}][{}] = {{".format(nbImg, len(testImage[0])*len(testImage[0][0])))
    ...
    for k in range(len(testImage)):
        for l in range(len(testImage[k])):
            for m in range(len(testImage[k][l])):
                lib.write(str(float(testImage[k][l][m])))
